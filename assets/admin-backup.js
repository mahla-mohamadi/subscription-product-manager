jQuery(document).ready(function ($) {
    //HTML Element where form steps and inputs must be shown
    const formBuilder = $('.form-steps');
    //HTML Element where form data must be saved
    const hiddenInput = $('#sproduct_form_data');
    //HTML Element of add step button
    const addStepBtn = $('#add-step-btn');
    //Get form data of steps and inputs that was saved before and was got from post meta before
    ...
    function renderForm() {
        //Empty the step and input container
        formBuilder.html('');
        console.log(formData);
        //Loop into steps and inputs - stepIndex is the number of step
        formData.forEach((step, stepIndex) => {
            //Defaul HTML structure of a new step with add input button
            const stepDiv = $(`
    ...
    function saveForm() {
        //Loop through current existing steps that was created
        $('.step').each(function (stepIndex) {
            //Loop through input row wrapper including type name and ...
            $(this).find('.input-item').each(function (inputIndex) {
                //Get input type (Dropdown Select)
                const inputType = $(this).find('.input-type').val();
                //Save input type in formData array
                formData[stepIndex].inputs[inputIndex].type = inputType;
                //Check if is checkbox or radio
                if (inputType === 'checkbox_group' || inputType === 'radio_group') {
                    const optionClass = inputType === 'checkbox_group' ? '.checkbox-option' : '.radio-option';
                    const options = [];
                    $(this).find(optionClass).each(function() {
                        const value = $(this).val().trim();
